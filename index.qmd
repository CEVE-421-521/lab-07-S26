...
The two functions below have the same structure as the unweighted metrics.
...
function weighted_expected_cost(df, weights)
    combine(groupby(df, :elevation_ft)) do gdf
        w = weights[gdf.trajectory]  # weights for this group's trajectories
        costs = gdf.total_cost
# Your code here
# Your code here
```
...
::: {.callout-note collapse="true" title="Hint"}
Compare to the unweighted versions: `mean(costs)` becomes `sum(costs .* w)`, and `mean(costs .< τ)` becomes `sum(w .* (costs .< τ))`.
The weights `w` replace the implicit $1/N$ from `mean`.
:::
